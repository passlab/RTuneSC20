...
double jacobi_omp(int n, int m, REAL dx, REAL dy, REAL alpha, REAL relax, REAL *u_p, REAL *f_p, REAL tol, int mits, long int* num_threads_p);

// Active Harmony tuning preparation
// init tuning session
hdesc_t* init_tuner()
{
    hdesc_t* hdesc;
    hdesc = ah_alloc();
    // read nothing from command-line
...
void advanced_config(hdef_t* hdef)
{
    ...
    // init search value, 50% of the search space
    //ah_def_cfg(hdef, "INIT_RADIUS", "0.50");

    //ah_def_layers(hdef, "agg.so");

    // run 10 times to get the median, do not need to implement in the original code
    //ah_def_cfg(hdef, "AGG_TIMES", "10");
    //ah_def_cfg(hdef, "AGG_FUNC", "median");
}

// connect reaserch to the tuning session
htask_t* new_search(hdesc_t* hdesc)
{
    ...
    // search mechanism: from 4 to 32, with stride of 2
    ah_def_int(hdef, "num_threads", 4, 32, 2, NULL);
...
#pragma omp single
    ...
    for (i=0; i<num_runs; i++) {
        elapsed += jacobi_omp(n, m, dx, dy, alpha, relax, uomp, f, tol, mits, &num_threads);
    }
    elapsed = elapsed / num_runs;
    printf("OpenMP (%ld threads) elapsed time(ms): %12.6g\n", num_threads, elapsed);
...
double jacobi_omp(int n, int m, REAL dx, REAL dy, REAL alpha, REAL omega, REAL *u_p, REAL *f_p, REAL tol, int mits, long int* num_threads_p) {
    int i, j, k;
    REAL error;
    REAL ax;
    REAL ay;
...
/********** Active Harmony ******************/
        ...
        ah_bind_int(htask, "num_threads", num_threads_p);
    while ((k <= mits)) {
        error = 0.0; 
	double perf, ah_tune_elapsed;
        
	while (!ah_converged(htask)) {	
	    ah_fetch(htask);
	    //printf("AH selected tuning parameter num_threads: %ld\n", *num_threads_p);
            omp_set_num_threads(*num_threads_p);

            ah_tune_elapsed = read_timer_ms();
	//printf("===================== iteration %d ===========================\n", k);
        /* Copy new solution into old */
...
#pragma omp parallel for private(j,i, resid) reduction(+:error)
	    ...
	    perf = -ah_tune_elapsed + read_timer_ms();
