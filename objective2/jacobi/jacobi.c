...
#pragma omp single
    ...
    elapsed = jacobi_omp(n, m, dx, dy, alpha, relax, uomp, f, tol, mits);
...
double jacobi_omp(int n, int m, REAL dx, REAL dy, REAL alpha, REAL omega, REAL *u_p, REAL *f_p, REAL tol, int mits) {
    int i, j, k;
    //REAL error;
    double error;
    REAL ax;
    ...
    k2[1] = 0.0;
    int side_walk = 0;
    double k3_limit = 0.0005;
    double k2_limit = 0.00000000000001; 
    while ((k <= mits) && (error > tol)) {
        error = 0.0;

        //printf("===================== iteration %d ===========================\n", k);
        RTune_begin(RTUNE_PERFORMANCE, NULL);
        /* Copy new solution into old */
...
#pragma omp parallel for private(j,i, resid) reduction(+:error)
        ...
        if (k%5 == 0) {
            ...
            val[0] = val[1];
            val[1] = error;
            //if (k2[1] > 0 && k2[1] < k2_limit) {
            if (k2[1] > 0 && (k2[0] - k2[1])/k2[0] < k3_limit) {
                if (side_walk < 15) {
                    side_walk += 1;
                    printf("Side walk! %d --- 15\n", side_walk);
                    //continue;
                }
                else {
                    printf("Stop early. Second derivative: %.15lg\n", k2[1]);
                    printf("Stop early. Third derivative: %.15lg\n", (k2[0] - k2[1])/k2[0]);
                    printf("Saved iterations based on mits: %d out of %d\n", mits-k, mits);
